...
const UserTable = ({ users, isPending, isRejected, isStaff, isClient, handleApprove, handleDeleteUser, handleRoleChange, handleReject, handleDeleteUserClick }) => {
                  ...
                  <TableCell>
                    {isPending ? (
                      <Stack spacing={2} direction="row">
                        ...
                        <Button variant="contained" color="error" onClick={() => handleReject(user._id)}>
                          Reject
                        </Button>
                      </Stack>
                    ) : isRejected ? (
                      <Stack spacing={2} direction="row">
                        <Button variant="contained" color="error" onClick={() => handleDeleteUserClick(user)}>
                          Delete
                        </Button>
                      </Stack>
                    ) : (
                      <Stack spacing={2} direction="row">
                        {editingUserId === user._id ? (
                          <Button variant="contained" color="primary" onClick={() => handleSaveClick(user._id)}>
                            Save
                          </Button>
                        ) : (
                          isStaff && (
                            <Button disabled={user._id === userId} variant="contained" color="primary" onClick={() => handleEditClick(user._id, user.role)}>
                              Edit
                            </Button>
                          )
                        )}
